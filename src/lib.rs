...
use types::*;
pub use utility::{AsOsStr, CChar, Error, Result, FP, RVA, URP};
use utility::{FPRef, RefSafe, URPConvert};

#[cfg(target_endian = "big")]
const E: ENDIANNESS_NOT_SUPPORTED = ();
...
impl<'data> Pe<'data> {
    // PRIVATE
    fn resolve_rva_raw<'a>(
        &self,
        rva: RVA<()>,
        length: u32,
        max_length: Option<&'a mut u32>,
    ) -> Result<FP<()>> {
        for section in self.sections {
                ...
                && rva < section.virtual_address + section.virtual_size
            {
                ...
                return Ok(section
                    .pointer_to_raw_data
                    .offset(rva.get() - section.virtual_address.get()));
            }
        }
    ...
    pub fn new(data: &'data [u8]) -> Result<Pe<'data>> {
        ...
        let pe_oh = match sig {
            OH_SIGNATURE_PE32 => {
                ...
                if pe_header.size_of_optional_header < s {
                    return Err(Error::NotPe);
                }
    ...
    pub fn get_directory<D: Directory>(&self) -> Result<&'data DataDirectory<D::Type>> {
        self.directories
            .get(D::entry() as usize)
            .ok_or(Error::DirectoryMissing)
            .map(|ddir| unsafe {
                transmute::<&'data DataDirectory<_>, &'data DataDirectory<_>>(ddir)
            })
    }

    pub fn get_directory_raw(
        &self,
        ...
        Ok(unsafe {
            transmute::<&'data DataDirectory<_>, &'data DataDirectory<_>>(
                &self.directories[entry as usize],
            )
        })
    }
    ...
    pub fn get_exports(&self) -> Result<Exports> {
        ...
        Ok(Exports {
            pe: self,
            ddir: ddir,
            edir: try!(self.ref_at(ddir.virtual_address)),
        })
    }
    ...
    pub fn get_relocations<'pe>(&'pe self) -> Result<RelocationIter<'pe, 'data>> {
        ...
        Ok(RelocationIter {
            pe: self,
            next_rblock: ddir.virtual_address,
            end: ddir.virtual_address + ddir.size,
        })
    }
}
...
impl<'pe, 'data: 'pe> Exports<'pe, 'data> {
    ...
    pub fn concretize_export_address<'a>(&self, addr: &'a RawExportAddress) -> ExportAddress<'a> {
        if addr.0 >= self.ddir.virtual_address
            && addr.0 < (self.ddir.virtual_address + self.ddir.size)
        {
    ...
    pub fn get_names(&self) -> Result<&'data [RVA<[CChar]>]> {
        self.pe
            .ref_slice_at(self.edir.name_pointer, self.edir.number_of_name_pointers)
    }
    ...
    pub fn get_ordinal_offsets(&self) -> Result<&'data [u16]> {
        self.pe
            .ref_slice_at(self.edir.ordinal_table, self.edir.number_of_name_pointers)
    }

    /// The outputs of this function is indexed by ordinal offset.
    pub fn get_export_addresses(&self) -> Result<&'data [RawExportAddress]> {
        self.pe.ref_slice_at(
            self.edir.export_address_table,
            self.edir.address_table_entries,
        )
    }

    pub fn lookup_symbol(&self, symbol: &str) -> Result<ExportAddress<'data>> {
        let pos = try!(try!(self.get_names())
            .iter()
            .position(|&name_rva| {
                self.pe
                    .ref_cstr_at(name_rva)
                    .ok()
                    .map_or(false, |cstr| cstr.as_os_str() == symbol)
            })
            .ok_or(Error::SymbolNotFound));
        let ordinal_offset = try!(self.get_ordinal_offsets())[pos];
        let export = try!(try!(self.get_export_addresses())
            .get(ordinal_offset as usize)
...
impl<'pe, 'data: 'pe> RelocationIter<'pe, 'data> {
    fn advance(&mut self) -> Result<(RVA<()>, &'data [Relocation])> {
        ...
        let relocs: &[Relocation] = try!(self.pe.ref_slice_at(
            self.next_rblock.offset(size_of::<RelocationBlock>() as u32),
            rblock.block_size / 2
        ));
